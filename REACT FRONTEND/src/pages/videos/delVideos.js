...
const DelVideos = ({flashFunc}) => {
    ...
    const classes = {
        ...
        "uVid": "h-16 w-16 sm:w-20 sm:h-20 md:w-24 md:h-24 m-auto mt-4",
    ...
    const txt = {
        "wnTxt": "Are you sure you want to delete this video?",
        "vdTxt": "Your browser does not support the video tag."
    }

    const getVideo = async () => {
        try {
            var { res, data } = await customFetcher(`videos/get/${id}`)
    ...
    const playFunc = () => {
        ...
        vid.currentTime = 0;
        vid.classList = ["h-16 w-16 sm:w-20 sm:h-20 md:w-24 md:h-24 m-auto mt-4 border-4 border-green-700"]
        vid.play();
    }
    ...
    const delFunc = async () => {
        try {
            const {res, data} = await customFetcher(`videos/delete/${id}`, {method: "DELETE"})
            flashFunc(data.message, res.ok)
            if (res.ok) {
                navigate(`/videos/${page == 1 ? page : page - 1}`, { replace: true })
            }
        }
        catch (e) {
            flashFunc(err.msg, err.val)
    ...
    useEffect(() => {
        getVideo()
        if (`${pathname}/${id}/${page}` != `delVideos/${id}/${page}`) {
            navigate(`/delVideos/${id}/${page}`, { replace: true });
        }
    }, [])
    ...
    return (  
        <div className={classes.container}>
            ...
            <img src={imgs[12]} alt={rd.alt} className={classes.bgVid}/>
            {video?.id && <div className={classes.cdCon}>
                <h2 className={classes.wN}>{txt.wnTxt}</h2>
                <video className={classes.uVid} id={rd.id}>
                    <source src={video.link}/>
                    <h4>{txt.vdTxt}</h4>
                </video>
                <p className={classes.vidTxt}>{video.name}</p>
